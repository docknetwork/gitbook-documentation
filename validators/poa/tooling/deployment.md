...
1. Building from source by cloning the Github repo and using cargo to build. Run the node by specifying the chain spec `danforth_raw.json` \(for testnet\) or `vulcan_raw.json` \(for mainnet\) as `--chain=<path to souce>/cspec/danforth_raw.json` or `--chain=<path to souce>/cspec/vulcan_raw.json`.
